        ...
        def wait_poll(timeout)
          ...
          loop do
            ...
            if any?
              ...
              msg = "Still no connection available, waiting: %d, connection in queue: %d" % [@num_waiting, @queue]
              Rollbar.info msg
            end

            elapsed = Time.now - t0
            if elapsed >= timeout
              msg = 'could not obtain a database connection within %0.3f seconds (waited %0.3f seconds), in queue %d' %
                [timeout, elapsed, @num_waiting]
